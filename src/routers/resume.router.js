...
import { validateResumeCreate, validateResumeUpdate, validateResumeLog } from '../middlewares/validations/resume.validtation.middleware.js';
...
/* 이력서 생성 API */
resumeRouter.post('/', accessMiddleware, validateResumeCreate, catchError(async (req, res) =>{
...
/* 이력서 전체 조회 API */
resumeRouter.get('/', accessMiddleware, catchError(async (req, res) =>{
...
/* 이력서 상세 조회 */
resumeRouter.get('/:resumeId', accessMiddleware, catchError(async (req, res) =>{
  const { userId, userInfo } = req.user;
...
/* 이력서 수정 API*/
resumeRouter.patch('/:resumeId', accessMiddleware, validateResumeUpdate, catchError(async (req, res)=>{
  const resumeData = req.body;
  const { userId } = req.user;
  const { resumeId }= req.params;
...
/* 이력서 삭제 API*/
resumeRouter.delete('/:resumeId', accessMiddleware, catchError(async(req, res)=> {
  const { userId } = req.user;
  const { resumeId } = req.params;
...
/* 이력서 지원 상태 변경 API */
resumeRouter.patch('/:resumeId/status', accessMiddleware, requireRoles(['RECRUITER']), validateResumeLog, catchError(async (req, res) => {
  const { userId } = req.user;
  const { resumeId } = req.params;
...
/* 이력서 로그 목록 조회 API */
resumeRouter.get('/:resumeId/logs', accessMiddleware, requireRoles(['RECRUITER']), catchError(async (req, res) => {
  const { resumeId } = req.params;
